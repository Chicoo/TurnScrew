...
namespace TurnScrew.Wiki.SearchEngine
{
    ...
    {
        private Dictionary<IDocument, SortedBasicWordInfoSet> _dictionary;
        private Dictionary<string, IDocument> _mappings; // Used for performance purposes
        ...
        public OccurrenceDictionary(int capacity)
        {
            ...
            _mappings = new Dictionary<string, IDocument>(capacity);
            _dictionary = new Dictionary<IDocument, SortedBasicWordInfoSet>(capacity);
        }

        /// <summary>
        /// Initializes a new instance of the <see cref="OccurrenceDictionary" /> class.
        /// </summary>
        public OccurrenceDictionary() : this(10) { }
        ...
        public List<DumpedWordMapping> RemoveExtended(IDocument key, uint wordId)
        {
            ...
            {
                IDocument mappedDoc = _mappings[key.Name];
                if (_mappings.Remove(target.Name))
                {
                    // Prepare the list of DumpedWordMapping objects
                    SortedBasicWordInfoSet set = _dictionary[mappedDoc];
                    ...
                    if (!_dictionary.Remove(target)) throw new InvalidOperationException("Internal data is broken.");
        ...
        private IDocument FindKey(IDocument key)
        {
            ...
            if (_mappings.TryGetValue(key.Name, out target)) return target;
            else return null;
        }
        ...
        public SortedBasicWordInfoSet this[IDocument key]
        {
            get
            {
                ...
                else return _dictionary[target];
            }
            set
            {
                if (key == null) throw new ArgumentNullException("key");
        ...
        public void CopyTo(KeyValuePair<IDocument, SortedBasicWordInfoSet>[] array, int arrayIndex)
        {
            ...
            int i = 0;
            foreach (KeyValuePair<IDocument, SortedBasicWordInfoSet> pair in _dictionary)
            {
