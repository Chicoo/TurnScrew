...
namespace TurnScrew.Wiki.SearchEngine
{
    ...
    public abstract class IndexStorerBase : IDisposable
    {
        ...
        /// </summary>
        protected IInMemoryIndex _index = null;

        /// <summary>
        /// <c>true</c> if the index data seems corrupted.
        /// </summary>
        protected bool _dataCorrupted = false;
        ...
        {
            _index = index ?? throw new ArgumentNullException("index");
            indexChangedHandler = new EventHandler<IndexChangedEventArgs>(IndexChangedHandler);
            _index.IndexChanged += indexChangedHandler;
        }
        ...
        /// </summary>
        public Exception ReasonForDataCorruption { get; protected set; } = null;
        ...
        public void LoadIndex()
        {
            lock (this)
            {
                ...
                if (!_dataCorrupted)
                {
                    try
                    {
                        _index.InitializeData(documents, words, mappings);
                    }
                    catch (Exception ex)
                    {
                        ReasonForDataCorruption = ex;
            ...
            lock (this)
            {
                ...
                switch (e.Change)
                {
                    ...
                    case IndexChangeType.DocumentRemoved:
                        if (!_dataCorrupted) DeleteData(e.ChangeData, e.State);
                        break;
                    default:
