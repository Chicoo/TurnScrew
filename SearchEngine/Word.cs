...
namespace TurnScrew.Wiki.SearchEngine
{
    ...
    public class Word
    {
        ...
        /// The occurrences.
        /// </summary>
        protected OccurrenceDictionary _occurrences;
        /// <summary>
        /// The word unique ID.
        /// </summary>
        protected uint _id;
        ...
        public Word(uint id, string text) : this(id, text, new OccurrenceDictionary(10)) { }
        ...
        public Word(uint id, string text, OccurrenceDictionary occurrences)
        {
            ...
            _id = id;
            _occurrences = occurrences ?? throw new ArgumentNullException("occurrences");
        }
        ...
        public int TotalOccurrences
        {
            get
            {
                ...
                lock (_occurrences)
                {
                    foreach (KeyValuePair<IDocument, SortedBasicWordInfoSet> pair in _occurrences)
                    {
        ...
        public void AddOccurrence(IDocument document, ushort firstCharIndex, ushort wordIndex, WordLocation location)
        {
            ...
            lock (_occurrences)
            {
                if (_occurrences.ContainsKey(document))
                {
                    // Existing document
                    _occurrences[document].Add(new BasicWordInfo(firstCharIndex, wordIndex, location));
                }
                else
                {
                    // New document
        ...
        public List<DumpedWordMapping> RemoveOccurrences(IDocument document)
        {
            ...
            lock (_occurrences)
            {
                if (_occurrences.ContainsKey(document)) return _occurrences.RemoveExtended(document, ID);
        ...
        public void BulkAddOccurrences(IDocument document, SortedBasicWordInfoSet positions)
        {
            ...
            lock (_occurrences)
            {
                if (_occurrences.ContainsKey(document))
                {
                    ...
                    else _occurrences[document] = positions;
                }
                else _occurrences.Add(document, positions);
            }
        }
