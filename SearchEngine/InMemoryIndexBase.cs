...
namespace TurnScrew.Wiki.SearchEngine
{
    ...
    public abstract class InMemoryIndexBase : IInMemoryIndex
    {
        ...
        /// </summary>
        protected string[] _stopWords = null;
        /// <summary>
        /// Contains the index catalog.
        /// </summary>
        protected Dictionary<string, Word> _catalog = null;
        /// <summary>
        /// The <see cref="BuildDocument" /> delegate.
        /// </summary>
        protected BuildDocument _buildDocument = null;
        ...
        {
            _stopWords = new string[0];
            _catalog = new Dictionary<string, Word>(5000);
        }
        ...
        public int TotalDocuments
        {
            get
            {
                ...
                lock (this)
                {
                    foreach (KeyValuePair<string, Word> pair in _catalog)
                    {
        ...
        public void InitializeData(DumpedDocument[] documents, DumpedWord[] words, DumpedWordMapping[] mappings)
        {
            ...
            if (_buildDocument == null) throw new InvalidOperationException("InitializeData can be invoked only when the BuildDocument delegate is set.");

            lock (this)
            {
                _catalog.Clear();
                _catalog = new Dictionary<string, Word>(words.Length);
                ...
                {
                    IDocument builtDoc = _buildDocument(doc);
        ...
        public int StoreDocument(IDocument document, string[] keywords, string content, object state)
        {
            ...
            words = Tools.RemoveStopWords(words, _stopWords);
            ...
            WordLocation location, out Word newWord, out DumpedWord dumpedWord)
        {
            ...
            lock (this)
            {
                ...
                if (!_catalog.TryGetValue(wordText, out word))
                {
        ...
        protected DumpedChange RemoveDocumentInternal(IDocument document)
        {
            ...
            foreach (string w in _catalog.Keys)
            {
                dm.AddRange(_catalog[w].RemoveOccurrences(document));
            }
            ...
            foreach (string w in _catalog.Keys)
            {
                if (_catalog[w].TotalOccurrences == 0) toRemove.Add(w);
            }
            dw = new List<DumpedWord>(toRemove.Count);
            foreach (string w in toRemove)
            {
                dw.Add(new DumpedWord(_catalog[w]));
                _catalog.Remove(w);
            }
        ...
        public SearchResultCollection Search(SearchParameters parameters)
        {
            ...
            using (IWordFetcher fetcher = new InMemoryIndexWordFetcher(_catalog))
            {
    ...
    {
        private Dictionary<string, Word> _catalog;
        ...
        public bool TryGetWord(string text, out Word word)
        {
            lock (_catalog)
            {
                return _catalog.TryGetValue(text, out word);
            }
        }
